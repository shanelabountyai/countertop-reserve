...
- **(Product)** Over-seating waste cap — how far over is acceptable? V1: configurable, default 2 seats; it is the difference between a full room and a wasted six-top. Built as `FloorPlan.overSeatCap`, applied in `fittingUnits` alongside `minParty`, set to 2 in `apps/web/lib/restaurant.ts`. *(resolved at close, 2026-09-21)*
- **(Product)** Should a released reservation be silently re-bookable by the same guest? V1: yes, via the re-book link; treat it as a new booking with a new allocation. Built as the `{bookLink}` slot in the `released` and `cancelled` templates, pointing at `/book` — the ordinary guest flow, so the re-book contends on the allocation constraint like any other booking and the released table is real inventory for walk-ins meanwhile. *(resolved at close, 2026-09-21)*
- **(Builder)** Where does this deploy? This PRD never named a target, unlike Countertop's. First answered at close as **nowhere, deliberately** — the argument being that a hosted instance only adds a live carrier, which Non-Goals exclude. **Reopened and reversed 2026-09-21 (V-014): `reserve.labintelligence.co`, Vercel + Neon, matching the sibling projects.** The original answer was sound about SMS and wrong about the demo: it assumed the carrier was the only thing a deployment buys, when the thing it actually buys is a link you can send. Every sibling (`eventoolkit`, `storage`, `talk4me`) is already a subdomain, so this project was the only one that needed its owner present to be seen. The seeded service stays the deliverable; the deployment just makes it reachable. *(reversed at V-014, 2026-09-21)*
- **(Ops)** Is a 21:00 quiet-hours start too early for a restaurant that seats until 22:00? Likely — the reminder cadence and quiet hours want operator input before Phase 3. *(resolved at V-009 kickoff: keep 21:00–09:00, but quiet hours hold back only what can wait for morning — a reply to the guest's own text, and anything about a reservation starting before the window ends (tonight's table), still send; reminders and release notices for later days defer to 09:00)*
