...
- **(Builder)** Where does this deploy? This PRD never named a target, unlike Countertop's. V1: **nowhere, deliberately** — the seeded 60-cover service (`npm run db:seed:demo`) exercises every screen and all seven ugly cases locally, and the only thing a hosted instance adds is a live carrier on the wire, which Non-Goals already exclude. P2's real SMS integration needs a public webhook URL and is the one thing that reopens this. *(resolved at close, 2026-09-21)*
